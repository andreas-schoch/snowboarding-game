...
export class DebugDrawer {
  ...
  constructor(scene: Phaser.Scene, private lineWidth: number = 2) {
    ...
    this.instance.DrawSegment = (vert1_p: number, vert2_p: number, color_p: number) => {
      ...
      this.graphics.lineStyle(this.lineWidth, rgbToHex(color), 1);
    ...
    this.instance.DrawPolygon = (vertices_p: number, vertexCount: number, color_p: number) => {
      ...
      this.graphics.lineStyle(this.lineWidth, rgbToHex(color), 1.0);
      this.graphics.strokePoints(vertices.map(v => ({x: v.x * ppm, y: -v.y * ppm})), true, true);
    };
...
function rgbToHex({r, g, b}: Box2D.b2Color): number {
  return (r << 16) | (g << 8) | b;
}
